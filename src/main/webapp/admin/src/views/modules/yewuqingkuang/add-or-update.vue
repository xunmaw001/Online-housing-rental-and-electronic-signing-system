    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="奖惩加减分" prop="jiangchengjiajianfen">
          <el-select v-model="ruleForm.jiangchengjiajianfen" placeholder="请选择奖惩加减分">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="奖惩加减分" prop="jiangchengjiajianfen">
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="评价月份" prop="pingjiayuefen">
          <el-input v-model="ruleForm.pingjiayuefen" 
              placeholder="评价月份" clearable  :readonly="ro.pingjiayuefen"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="评价月份" prop="pingjiayuefen">
              <el-input v-model="ruleForm.pingjiayuefen" 
                placeholder="评价月份" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  computed: {
    zongdefen: {
      get: function () {
        return 0+parseFloat(this.ruleForm.gongzuotaidu==""?0:this.ruleForm.gongzuotaidu)+parseFloat(this.ruleForm.yewujineng==""?0:this.ruleForm.yewujineng)+parseFloat(this.ruleForm.gongzuojixiao==""?0:this.ruleForm.gongzuojixiao)+parseFloat(this.ruleForm.jiangchengjiajianfen==""?0:this.ruleForm.jiangchengjiajianfen) || 0
      }
    },
  },
  created() {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.pingjiadengji = true;
          ...
          if(o=='pingjiayuefen'){
            this.ruleForm.pingjiayuefen = obj[o];
	    this.ro.pingjiayuefen = true;
            continue;
          }
        }
      }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yewuqingkuang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yewuqingkuangCrossAddOrUpdateFlag = false;
                  this.parent.search();
